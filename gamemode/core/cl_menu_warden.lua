...
function JB.MENU_WARDEN()
	...
	if LocalPlayer().GetWarden and LocalPlayer():GetWarden() and tobool(JB.Config.wardenControl) then
		...
		frame:SetTitle("Controles de Warden");
		frame:SetWide(320);
		
		local yBottom = 30;
		
		local lbl = Label("Opciones de la partida",frame);
				lbl:SetFont("JBLarge");
		...
		addButton(tostring(IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_PVPDamage()),"Friendlyfire para los prisioneros",function() RunConsoleCommand("jb_warden_changecontrol","PVP",tostring(not (IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_PVPDamage()))) end);
		addButton(tostring(IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_ItemPickup()),"Levantar objetos",function() RunConsoleCommand("jb_warden_changecontrol","Pickup",tostring(not (IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_ItemPickup()))); end);
		
		yBottom = yBottom+16;
		local lbl = Label("Spawneo de objetos",frame);
				lbl:SetFont("JBLarge");
				lbl:SetColor(JB.Color["#EEE"]);
				lbl:SizeToContents();
				lbl:SetPos(15,yBottom + 15);
		...
		addButton(false,"Spawnear munición",function() RunConsoleCommand("jb_warden_spawn","AmmoBox") end);
		addButton(false,"Spawnear caja rompible",function() RunConsoleCommand("jb_warden_spawn","Crate") end);
		addButton(false,"Spawnear bloqueo",function() RunConsoleCommand("jb_warden_spawn","Blockade") end);
	...
	elseif JB.State == STATE_SETUP and not IsValid(JB:GetWarden()) then
		frame = vgui.Create("JB.Frame");
		frame:SetTitle("Promocionarte a warden");
		
		local lbl = Label("No te hagas warden si no tenés microfono.",frame);
		lbl:SetFont("JBSmall");
		lbl:SetColor(color_text);
		lbl:SizeToContents();
		lbl:SetPos(15,30+15);
