...
function JB.MENU_HELP_OPTIONS()
	...
	btn_credits:SetText("Acerca del gamemode (Inglés)")
	btn_credits.y = 120;
	...
	btn_options.OnMouseReleased = function()
		...
		local lbl=Label("Opciones",right);
		lbl:SetPos(20,20);
		lbl:SetFont("JBLarge");
		lbl:SizeToContents();
		lbl:SetColor(JB.Color.white);
		...
		for k,v in ipairs{
			{"jb_cl_option_toggleaim","toggle","Mira toggleable (default: Right Mouse)"},
			{"jb_cl_option_togglecrouch","toggle","Agache toggleable (default: CTRL)"},
			{"jb_cl_option_togglewalk","toggle","Caminata toggleable (default: ALT)"},
			{"jb_cl_option_always_spectate","toggle","Espawnear como espectador despues de entrar al servidor"}
		} do
