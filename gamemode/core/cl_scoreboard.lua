...
vgui.Register("JBScoreboard.PlayerRow",{
	...
	PaintOver = function(self,w,h)
		...
		elseif self.Player.GetRebel and self.Player:GetRebel() then
			draw.SimpleText("Revelado","JBLargeBold",self:GetWide()/2 + 30,26,color_faded,1)
		end
	end
},"Panel");
...
vgui.Register("JBScoreboard",{
	Init = function( self )
		...
		self.Host:SetText("Un modo de juego hecho por Excl, actualmente hosteando la partida Adler de GMA!");
