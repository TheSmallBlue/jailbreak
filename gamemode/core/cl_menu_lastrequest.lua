...
function JB.MENU_LR()
	...
	if (JB.State ~= STATE_PLAYING and JB.State ~= STATE_SETUP and JB.State ~= STATE_LASTREQUEST) or JB.AlivePrisoners() > 1 or JB:AliveGuards() < 1 or not LocalPlayer():Alive() then
		...
		local lbl = Label("El Last Request es la ultima chance de que los prisioneros ganen si todos los intentos de rebelión fracasaron.\nConsiste en un pequeño minijuego elegido por el prisionero contra el guardia a su elección. \n\nSolo podes empezar un Last Request si sos el ultimo prisionero vivo y si la ronda sigue activa.",frame);
		lbl:SetFont("JBSmall");
