...
function JB.Gamemode.JailBreakStartMapvote(rounds_passed,extentions_passed)
	hook.Add("JailBreakStartMapvote",MapVote.Start())
	return false
end
...
function JB:NewRound(rounds_passed)
	...
	elseif CLIENT and IsValid(LocalPlayer()) then
		notification.AddLegacy("Ronda "..rounds_passed,NOTIFY_GENERIC);
...
function JB:EndRound(winner)
	...
	elseif CLIENT then
		notification.AddLegacy(winner == TEAM_PRISONER and "Ganan los prisioneros" or winner == TEAM_GUARD and "Ganan los guardias" or "Empate",NOTIFY_GENERIC);
	end
